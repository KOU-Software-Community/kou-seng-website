...
export default function AdminTechnicalTeam({ category }: { category?: string }) {
    ...
    useEffect(() => {
        ...
        const params: any = { type: TYPE, category: category, page, limit: PAGE_SIZE, search: searchValue ?? '' };
    ...
    useEffect(() => {
        ...
        const handle = setTimeout(() => {
            setPage(1);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const params: any = { type: TYPE, category: category, page: 1, limit: PAGE_SIZE, search: searchValue ?? '' };
            if (statusFilter) params.status = statusFilter;
            listSubmissions(params);
        }, 400);
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const params: any = { type: TYPE, category: category, page: 1, limit: PAGE_SIZE, search: '' };
        if (statusFilter) params.status = statusFilter;
